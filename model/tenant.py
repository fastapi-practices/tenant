...
class Tenant(Base):
    ...
    __table_args__ = (
        sa.Index('ix_sys_tenant_pkg_id', 'package_id'),
        sa.UniqueConstraint('name', 'deleted', name='uk_sys_tenant_name_deleted'),
        sa.UniqueConstraint('code', 'deleted', name='uk_sys_tenant_code_deleted'),
        sa.UniqueConstraint('domain', 'deleted', name='uk_sys_tenant_domain_deleted'),
    )

    id: Mapped[id_key] = mapped_column(init=False)
    name: Mapped[str] = mapped_column(sa.String(64), index=True, comment='租户名称')
    code: Mapped[str] = mapped_column(sa.String(64), index=True, comment='租户编码')
    ...
    domain: Mapped[str | None] = mapped_column(sa.String(256), default=None, index=True, comment='租户域名')
